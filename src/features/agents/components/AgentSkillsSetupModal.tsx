...
export const AgentSkillsSetupModal = ({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`Configuration de ${skill.name}`}
      onClick={onClose}
    >
      <div
        ...
        <div className="flex items-start justify-between gap-3 px-6 py-5">
          <div className="min-w-0">
            ...
            <div className="mt-2 text-[10px] text-muted-foreground/80">
              Les modifications affectent tous les agents sur ce gateway.
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-3 px-6 pb-3 text-[11px] text-muted-foreground">
          ...
          {skill.blockedByAllowlist ? (
            <div className="text-[10px] text-muted-foreground/80">
              Bloqué par la politique des skills intégrés (`skills.allowBundled`).
            </div>
          ) : null}
          ...
          <div className="space-y-2 rounded-md border border-border/60 bg-surface-1/65 px-3 py-3">
            {installOption ? (
              ...
              >
                {busyForSkill ? "En cours..." : installOption.label}
              </button>
            ) : null}
            <button
              type="button"
            ...
            {skill.primaryEnv ? (
              <>
                <input
                  ...
                  placeholder={`Définir ${skill.primaryEnv}`}
                  aria-label={`Clé API pour ${skill.name}`}
                />
                <button
                  type="button"
                  className="ui-btn-secondary w-full px-3 py-2 text-[10px] font-medium disabled:cursor-not-allowed disabled:opacity-65"
                ...
                >
                  {busyForSkill ? "En cours..." : `Enregistrer ${skill.primaryEnv}`}
                </button>
              </>
            ) : null}
            {canDeleteSkill ? (
              <button
                ...
                onClick={() => {
                  const approved = window.confirm(
                    `Retirer ${skill.name} du gateway ? Cela affecte tous les agents.`
                  );
                  if (!approved) {
                    return;
                  }
